...
/**
 * Returns HTML to display course categories as tiles on the front page.
 *
...
public function frontpage_categories_list() {
    ...
    $subcategories = $topcategory->get_children();

    if (empty($subcategories)) {
        return '';
    }

    $output = html_writer::start_div('category-tiles-container d-flex flex-wrap justify-content-center gap-4 mt-4');
    ...
    if (!$category->visible) {
        ...
        $caturl = new \moodle_url('/course/index.php', ['categoryid' => $category->id]);
        $coursescount = $category->get_courses_count();

        // --- Get theme-uploaded image for this category ---
        $storedfile = $this->page->theme->setting_file_url('categoryimage_' . $category->id, 'categoryimage_' . $category->id);
        $noimgurl = $this->output->image_url('no-image', 'theme');
        $imgurl = $storedfile ?: $noimgurl;

        // --- Build the tile ---
        $output .= html_writer::start_div('category-tile card shadow-sm border-0 text-center', ['style' => 'width: 220px;']);
        ...
        $output .= html_writer::tag('h5',
            html_writer::link($caturl, $catname, ['class' => 'stretched-link text-decoration-none text-dark']),
            ['class' => 'card-title fw-semibold']
        );
        $output .= html_writer::tag('p', get_string('numberofcourses') . ': ' . $coursescount, ['class' => 'text-muted small']);
        $output .= html_writer::end_div(); // card-body
        $output .= html_writer::end_div(); // category-tile
    }

    $output .= html_writer::end_div(); // container

    return $output;
}
public function export_for_template(\renderer_base $output) {
    return [
        'categories_html' => $this->frontpage_categories_list(),
    ];
}
    /**
...
/**
     * Frontpage banner exactly like the provided design.
     *
     * @return string HTML
     */
    public function frontpage_stats_banner(): string {
        global $DB;

        // Users: aktywni, potwierdzeni, nieusunięci.
        $totalusers = $DB->count_records_select('user',
            "deleted = 0 AND suspended = 0 AND confirmed = 1"
        );
	$totalusers=$totalusers-1;
        // Courses: widoczne, bez kursu głównego (id=1).
        ...
        // Activities: widoczne moduły w widocznych kursach (bez kursu id=1).
        ...
        // Format liczb 12 345 (spacja tysięcy).
        $fmt = function(int $n): string {
            return number_format($n, 0, ',', ' ');
        };

        // Sekcja (układ: tytuł po lewej, trzy staty po prawej).
        $html  = html_writer::start_div('stats-banner'); // tło i padding w CSS

        // Lewa kolumna: tytuł (dwuliniowy, złoty akcent).
        $html .= html_writer::start_div('stats-left');
        $html .= html_writer::tag('h2',
            'WYBRANE<br>' . html_writer::tag('span', 'LICZBY'),
            ['class' => 'stats-title', 'aria-label' => 'Wybrane liczby']
        );
        $html .= html_writer::end_div();

        // Prawa kolumna: 3 elementy z separatorami.
        $html .= html_writer::start_div('stats-container d-flex justify-content-center align-items-start');

        // 1) Użytkownicy.
        $html .= html_writer::start_div('stat-item');
        $html .= html_writer::tag('div', '', ['class' => 'fa fa-user-circle stat-icon', 'aria-hidden' => 'true']);
        $html .= html_writer::tag('div', $fmt($totalusers), ['class' => 'stat-number']);
        $html .= html_writer::tag('div', '', ['class' => 'stat-hr', 'aria-hidden' => 'true']);
        $html .= html_writer::tag('div', get_string('users', 'moodle'), ['class' => 'stat-label']);
        $html .= html_writer::end_div();

        // 2) Kursy.
        $html .= html_writer::start_div('stat-item');
        $html .= html_writer::tag('div', '', ['class' => 'fa fa-graduation-cap stat-icon', 'aria-hidden' => 'true']);
        $html .= html_writer::tag('div', $fmt($totalcourses), ['class' => 'stat-number']);
        $html .= html_writer::tag('div', '', ['class' => 'stat-hr', 'aria-hidden' => 'true']);
        $html .= html_writer::tag('div', get_string('courses'), ['class' => 'stat-label']);
        $html .= html_writer::end_div();

        // 3) Aktywności.
        $html .= html_writer::start_div('stat-item');
        $html .= html_writer::tag('div', '', ['class' => 'fa fa-puzzle-piece stat-icon', 'aria-hidden' => 'true']);
        $html .= html_writer::tag('div', $fmt($totalactivities), ['class' => 'stat-number']);
        $html .= html_writer::tag('div', '', ['class' => 'stat-hr', 'aria-hidden' => 'true']);
        // jeśli nie masz tłumaczenia, możesz podmienić na stały tekst 'AKTYWNOŚCI'
        $html .= html_writer::tag('div', get_string('activities'), ['class' => 'stat-label']);
        $html .= html_writer::end_div();

        $html .= html_writer::end_div();   // .stats-container
        $html .= html_writer::end_div();   // .stats-banner
